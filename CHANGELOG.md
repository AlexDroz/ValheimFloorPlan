...
- **Designer: no tool is selected by default on startup.** Previously the Designer always started with `Floor2x2` pre-selected, which could lead to accidental placements before you'd chosen a tool. The canvas now starts with nothing selected.
...
- **Designer: FlexiWalls now use the same select-then-delete flow as furniture pieces.** Placed FlexiWalls show a midpoint handle that can be clicked at any time (no need to have the FlexiWall tool active) to select the wall — shown with the same dashed yellow outline used for selected furniture — and then removed with `Delete`/`Backspace`. Right-click on a FlexiWall handle no longer deletes it; right-click is now purely a cancel action everywhere, matching `Escape`. While the FlexiWall tool is active, the handle still drags to reshape the curve as before.
- **Designer: selecting a placed piece via its handle now also activates the matching tool.** Clicking a furniture piece's or FlexiWall's handle switches the active tool radio to that piece's type — for FlexiWalls, this is what re-enables the handle's drag-to-reshape behavior immediately after selecting it.
...
- **Fixed: `Delete`/`Backspace` no longer silently fails right after picking a tool from the menu.** Clicking a tool radio button left it holding keyboard focus, which caused the deletion shortcut to be swallowed before it ever reached the canvas — you had to place a piece first to "unstick" it. Hovering the grid now always lets `Delete`/`Backspace` through, regardless of which control last had focus, matching the same precedence already used for arrow-key rotation.
